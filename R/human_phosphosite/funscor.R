...
phospho.annotation <- phospho.raw %>% select(-id) %>% mutate(Ids = Ids, id = id) %>% select(-contains("Subject")) %>% select(Ids, id, everything()) %>% filter(id %in% rownames(grand.tab))

# To run funScor, generate a table containing protein accession, residue position and residue amino acid
funscor <- phospho.annotation %>% select(Protein, Position, Amino.acid) %>% `colnames<-`(c("acc", "position", "residue")) %>% `rownames<-`(c(1:nrow(.))) %>% filter(!is.na(acc) & !is.na(position)) %>% distinct()
...
rm(ST_model, ST_features, ST_scores, Y_model, Y_features, Y_scores, phospho.annotation)
